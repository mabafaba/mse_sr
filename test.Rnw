...
\begin{abstract}
Entropy relates the fast, microscopic behaviour of the elements in a system to its slow, macroscopic state. We propose to use it to explain how, as complexity theory suggests, small scale decisions of individuals form cities. For this, we offer the first interpretation of urban entropy for cities that takes into account that interaction between different places should be reflected in interdependently entangled states, and do so with a multiscale approach.
...
Theoretically, the continuous phase space $\mu^{(N+1)C}$ has a unique dimension for each of the $C$ characteristics for all $N$ scales plus the not aggregated scale $d_0$. It is the the space of all possible place state matrices $\Psi \in \mu$.
