...
\begin{figure}[ht]
   ...
   \caption{phase space distributions for patterns a) - \Sexpr{synthfig_letters[length(synthfig_letters)]} at scales of \Sexpr{lags[synth_phasespace_plot_scales[1]]} and \Sexpr{lags[synth_phasespace_plot_scales[2]]} pixels neighbourhood side length}
