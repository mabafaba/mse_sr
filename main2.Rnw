...
Let us now test this method on a range of simulated patterns in the following section.
...
\begin{figure}[ht]
   \centering
    \includegraphics[width=0.8\textwidth]{./code/output/syntheticgeospace_frequency.jpg}
...
\textbf{pattern \Sexpr{synthfig_letters["segmed"]} to \Sexpr{synthfig_letters["seglarge"]} Segregated:}\\
