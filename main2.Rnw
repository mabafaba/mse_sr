...
Entropy is a measure of how evenly observations in a system are distributed in the space of possible states they can be in. This space is called the phase space. Fore more even distributions in the phase space there are more permutations of which exact observation is in which state, which is why they can be overwhelmingly more likely, and so we expect entropy to increase over time. Existing measures of entropy in an urban context are either non-spatial, or use the mathematical properties of entropy as a proxy for spatial evenness. In this paper, we attempt to formulate a measure of entropy that is instead conceptually consistent with entropy in statistical mechanics and an understanding of cities as complex systems: \newline
